...
class DataTransformation:
    ...
    def initiate_data_transformation(self) -> DataTransformationArtifact:
        try:
            ...
            train_transfom: transforms.Compose = self.transforming_training_data()
            
            test_transfom: transforms.Compose = self.transforming_testing_data()
            
            os.makedirs(self.data_transformation_config.artifact_dir, exist_ok=True)
            
            joblib.dump(
                train_transfom, self.data_transformation_config.train_transforms_file
            )
            
            joblib.dump(
                test_transfom, self.data_transformation_config.test_transforms_file
            )
            
            train_loader, test_loader = self.data_loader(
                train_transform=train_transfom, test_transform=test_transfom
            )
